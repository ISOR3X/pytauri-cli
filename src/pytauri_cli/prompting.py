...
def print_questions(choices, index):
    for i, (label, meta) in enumerate(choices):
        selected = i == index
        prefix = "> " if selected else "  "

        sys.stdout.write(Command.CLEAR_LINE.value)
        if selected:
            sys.stdout.write(colored(prefix + label, Color.GREEN))
            if meta:
                sys.stdout.write(colored(f" ({meta})", Color.DIM))
        else:
...
def select(question: str, choices: list[tuple[str, str | None]]):
    """
